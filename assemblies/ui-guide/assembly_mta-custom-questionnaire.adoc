...
* If the application or archetype has the `Language/Java` tag, the {ProductShortName} includes the `What is the main JAVA framework used in your application?` question in the questionnaire.
+
[source,yaml]
----
...
...
* If the application or archetype has the `Deployment/Serverless` and `Architecture/Monolith` tag, the {ProductShortName} excludes the `Are you currently using any form of container orchestration?` question from the questionnaire:
+
[source,yaml]
----
...
...
If the {ProductShortName} selects this answer during an assessment, it automatically applies tags to the application or archetype. Because these tags are transitive, the {ProductShortName} removes them if you discard the assessment. The following elements define each tag:
+
--
