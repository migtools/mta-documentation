:_newdoc-version: 2.18.3
:_template-generated: 2025-05-28

ifdef::context[:parent-context-of-understanding-dl: {context}]

:_mod-docs-content-type: ASSEMBLY

ifndef::context[]
[id="understanding-dl"]
endif::[]
ifdef::context[]
[id="understanding-dl_{context}"]
endif::[]
= Understanding {mta-dl-full}

:context: understanding-dl

[role="_abstract"]
Understand the {mta-dl-full} extension features, use cases, configurations, and the benefits of using the extension with a large language model (LLM) to generate AI-assisted code resolutions when you want to refactor large codebases.

include::con_intro-to-developer-lightspeed.adoc[leveloffset=+1]

include::con_dl-context.adoc[leveloffset=+1]

include::con_solution_server.adoc[leveloffset=+1]

include::con_solution-server-and-large-language-model-integration.adoc[leveloffset=+1]

include::con_agent-ai.adoc[leveloffset=+1]

include::con_agent-mode-guardrails.adoc[leveloffset=+1]

include::ref_agent-mode-validation-checks.adoc[leveloffset=+1]

include::con_agent-mode-autocorrection.adoc[leveloffset=+1]

include::proc_enabling-agent-mode.adoc[leveloffset=+1]

include::ref_dl-benefits.adoc[leveloffset=+1]


ifdef::parent-context-of-understanding-dl[:context: {parent-context-of-understanding-dl}]
ifndef::parent-context-of-understanding-dl[:!context:]

:!understanding-dl:
