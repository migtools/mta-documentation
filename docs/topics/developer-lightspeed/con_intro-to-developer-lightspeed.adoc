...
The Solution Server acts as an institutional memory that stores changes to source codes after analyzing applications in your organization. This helps you to use the recurring patterns of solutions for issues that repeat in many applications.
...
Works with the LLM to generate fix suggestions. The LLM displays the reasoning transcript and files to be changed.
