...
[role="_abstract"]
Review the prerequisites required to successfully use the generative AI features in the {mta-dl-plugin} Visual Studio Code extension.
...
The {ProductShortName} Operator is mandatory if you plan to enable the Solution Server that works with the large language model (LLM) for generating code changes. Use the Operator to log in to the `openshift-mta` project where you must enable the Solution Server in the Tackle custom resources (CR).
...
| {ocp-name} AI platform| Models that you deploy in an {ocp-name} AI cluster and access by using Open AI-compatible API.
