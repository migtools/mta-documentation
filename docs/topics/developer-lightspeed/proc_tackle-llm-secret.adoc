...
* You configured an LLM API key secret in your {ocp-name} cluster to produce the resources necessary for the Solution Server. If you do not configure the LLM API key secret, {mta-dl-plugin} does not create the resources necessary to run the Solution Server.
...
[subs="+quotes"]
----
oc get secret kai-api-keys -o yaml
----