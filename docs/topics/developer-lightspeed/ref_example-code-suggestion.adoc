...
[role="_abstract"]
Generate code fixes for a Java application you want to migrate to the `quarkus` target technology. To stream code changes that resolve issues in the code, use the Agent mode and `my-model` as the large language model (LLM) that you deployed in {ocp-name} AI.

.Procedure

. Open the `my-Java` project in Visual Studio Code.

. Download the {mta-dl-full} extension from the Visual Studio Code marketplace.
