...
= LLM provider setting configurations
...
You can find the provider settings file in the {mta-dl-plugin} Visual Studio Code extension.
...
[NOTE]
====
To verify that you have command line access to the Amazon Web Services (AWS) services before you proceed with the `provider-settings` configurations, use the AWS CLI.
====
