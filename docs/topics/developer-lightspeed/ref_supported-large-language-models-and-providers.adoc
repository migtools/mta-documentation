...
[role="_abstract"]
Reviewing the supported large language models (LLMs) and providers ensures an accurate system configuration. {mta-dl-full} ({ProductShortName}) supports several options.

The following table lists the LLM providers and example models. You can configure these in the Tackle custom resource (CR).
