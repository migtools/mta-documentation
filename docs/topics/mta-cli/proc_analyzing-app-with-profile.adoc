...
[subs="+quotes, macros"]
....
$ *mta-cli config sync --url \https://github.com/_<my-app-repository>_ --application-path _<path_to_my_app>_ --insecure*
....
+
[NOTE]
====
...
[TIP]
====
For Java applications, use the `full` mode while running an analysis. For non-Java applications, the analysis mode must be `source-only`. 
====
+
[NOTE]
====
