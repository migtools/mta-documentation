...
[role="_abstract"]To efficiently assess the migration effort for a portfolio of applications, you can analyze multiple applications simultaneously by using the {ProductFullName} command-line interface (CLI). This bulk analysis generates reports for all specified applications. You can use the reports to identify…
