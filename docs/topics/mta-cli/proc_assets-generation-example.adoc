...
[role="_abstract"]
You can use the {ProductFullName} command-line interface to generate static assets for offline review. Use the following example to validate the command syntax and understand the expected output structure.
...
Assume that the `cf-nodejs-app.yaml` is located in the same directory as the {ProductShortName} {CLIName} binary. If the CF application manifest location is different, you can also enter the location path to the manifest as the `input`.
...
. Generate the deployment manifests in the `newDir` directory by using the `discover.yaml` file:
+
[subs="+quotes"]
----
