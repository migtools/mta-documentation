...
[role="_abstract"]
You can help the {ProductFullName} to cover most application builds and server configurations, including yours.
...
* Try {ProductName} on your application. Report any issues you encounter. {ProductShortName} uses Jira as its issue tracking system. If you encounter an issue when using {ProductShortName}, submit a link:{JiraMTAURL}[Jira issue].
* Contribute to the {ProductName} rules repository:
** Write a {ProductName} rule to identify or automate a migration process.
** Create a test for the new rule.
+
For more information, see link:{mta-URL}/configuring_and_using_rules_for_an_mta_analysis/index[Configuring and using rules for an MTA analysis].
