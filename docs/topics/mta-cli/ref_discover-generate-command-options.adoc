...
.6+a|`generate helm`
a|
a|Generate deployment manifests by using the Helm template.

a|`--chart-dir`
a|Specify a directory that contains the Helm chart.

a|`--input`
a|Specify a location of the _<discovery-manifest-name>_.yaml file to generate the deployment manifests.
