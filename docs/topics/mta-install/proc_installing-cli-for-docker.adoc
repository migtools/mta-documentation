...
.. Run the installation program by double-clicking the `Docker_Desktop_Installer.exe` file. 
+
...
. Ensure that Docker is running Windows containers and that the `OS/Arch` is set to `windows/amd64`:
+
[subs="+quotes"]
----
...
* link:https://docs.docker.com/desktop/install/windows-install/[Install Docker Desktop on Windows]
