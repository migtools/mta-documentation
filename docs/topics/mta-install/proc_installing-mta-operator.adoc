...
[role="_abstract"]
To create the {ProductFullName} instances, you must install the {ProductShortName} Operator first.

The {ProductShortName} Operator is a structural layer that manages resources deployed on Red Hat OpenShift, such as the database, front end, and back end, to automatically create an {ProductShortName} instance.
...
* You created one `ReadWriteOnce` (RWO) persistent volume (PV), an optional RWO PV for the Solution Server database, and two conditional `ReadWriteMany` (RWX) PVs.
...
. Click *{ProductFirstRef} Operator*.
. Click *Install*.
...
+
Alternatively, to work directly with the YAML file, click the *YAML* view and review the CR settings listed in the `spec` section of the YAML file. You can create one Tackle CR in a namespace.
+
