...
[role="_abstract"]
You can analyze your projects with the {ProductShortName} plugin by creating a run configuration and running an analysis. You can create multiple run configurations to run against each project you import to IntelliJ IDEA.
...
** *CLI*: Enter the path to the command-line interface (CLI) executable. For example: `$HOME/{LC_PSN}-cli-{ProductDistributionVersion}/bin/{mta-cli}`.
...
** *Reports* opens the {ProductShortName} report, which describes any issues you need to address before you migrate or modernize your application. For more information, see link:{ProductDocUserGuideURL}#reviewing-analysis-reports_cli-guide[Reviewing the reports] in the _{UserCLIBookName}_.
