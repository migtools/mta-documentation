...
[role="_abstract"]
The `BuildConfigToBuildsPlugin` converts legacy Red{nbsp}Hat OpenShift `BuildConfig` resources (`build.openshift.io/v1`) into Kubernetes-native build custom resources (`shipwright.io/v1beta1`) as part of the `mta-ops transform` pipeline. This offline transformation generates reviewable, GitOps-ready manifests without communicating with a live cluster.
