...
All operations output directly to your local disk. The pipeline does not modify the source or target clusters automatically.

The tool migrates stateless workloads and their associated cluster-scoped dependencies. These dependencies include role-based access controls (RBAC), Custom Resource Definitions (CRDs), and SecurityContextConstraints (SCCs).
