...
= The workload migration pipeline
...
Validation stage::
An optional but recommended preflight check that queries the target cluster discovery API to verify Group, Version, and Kind (GVK) compatibility. If the destination cluster lacks a required API version or Custom Resource Definition, the validator logs the incompatible resource in a failure directory and suggests available version updates.
