...
The `mta-ops transform` command processes exported manifests through sequential plugin stages. The pipeline produces a standard Kustomize layout. The pipeline isolates each execution step into a dedicated, numbered subdirectory under the central `transform/` directory. This open structure allows you to inspect the inputs, generated patches, and outputs of each plugin individually.
