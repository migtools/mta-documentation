...
** For Git: this might be any reference; `commit-hash`, `branch` or `tag`.
** For *Subversion*: this might be a fully qualified path to a branch or tag, for example, `branches/stable` or `tags/stable`. This must be blank when the Source repository URL includes the branch.
