...
** *Subversion*: The URL to the root of the repository or a fully qualified URL which optionally includes the branch and nested directory. When fully qualified, the *Branch* and *Root path* must be blank.
* *Branch*: An application code branch in the repository (optional).
** *Git*: Any reference, for example, `commit-hash`, `branch`, or `tag`.
** *Subversion*: A fully qualified path to a branch or tag, for example, `branches/stable` or `tags/stable`. This must be blank when the *Source repository* URL includes the branch.
* *Root path*: A root path inside the repository for the target application (optional).
** *Subversion*: The field must be blank when the *Source repository* URL includes the root path.
