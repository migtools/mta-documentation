...
[role="_abstract"]
Associate applications that you created in {ProductFullName} with their source platform instances, such as Cloud Foundry (CF), by configuring the application's coordinates.
...
. On the *Application Inventory* page, click the edit icon (image:icon_edit.png[edit icon]) in an application. The *Update application* dialog is displayed.
