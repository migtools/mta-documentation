...
A migrator can analyze more than one application at a time against more than one transformation target in the same analysis. 
...
. Select *Manual selection* in the *Mode* section for the analysis. You can complete the configuration for an analysis step-by-step with a unique configuration.
