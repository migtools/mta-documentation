...
[role="_abstract"]
As an Architect, you can configure a profile that has the configuration required to analyze an application for a specific migration target.
...
* Excluded rules labels. Rules with these labels are not processed. Add or delete as required.
