...
[id="creating-migration-waves_{context}"]
= Creating a migration wave

[role="_abstract"]
To schedule and track your modernization efforts, create migration waves in the {ProductFullName} user interface (UI).

A migration wave is a group of applications that you can migrate on a given schedule. You can track each migration by exporting a list of the wave's applications to the Jira issue management system. This automatically creates a separate Jira issue for each application of the migration wave.
...
. Click *Create*. The new migration wave is displayed in the list of existing migration waves.
