...
[role="_abstract"]
To avoid manually setting credentials for a large list of applications, set default credentials in the {ProductFullName} user interface (UI). 

You might want to import a large list of applications to the {ProductShortName} UI. Each of these applications requires credentials for both the language and technology discovery tasks. Instead of manually defining credentials for each application, you can set credentials that will be used by default. 
...
.. Click the Options menu ({kebab}) to the right of the credential you want to set as default.
