...
. You can retrieve configuration for one or more applications.
+
...
... Select the target applications or click the Select all option ({select-all-menu}).
...
. Click an application row to open the application details pane and click the *Platform* tab.

//are sensitive information such as passwords hashed?
. Review the discovery manifest with the retrieved configuration and download the JSON configuration.
