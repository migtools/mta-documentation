...
----
$ sudo mv <mta_cli_zip>/<os>-mta-cli /usr/bin
----
.. For example, to your `$PATH`:
+
[source,terminal,subs="attributes+"]
----
$ mv <mta_cli_zip>/<os>-mta-cli <PATH>
----

. Move the extracted requirements from the `mta-cli.<os>` zip to the `.kantra` directory inside your `$HOME` directory. For example:
+
[source,terminal,subs="attributes+"]
----
