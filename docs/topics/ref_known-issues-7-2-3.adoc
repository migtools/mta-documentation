...
.{ProductShortName} {CLIName} analysis prolongs for multi-module binaries when dependency resolution fails
When you analyze multi-module binaries in {ProductShortName} {CLIName}, the analysis takes long to complete if dependency resolution fails. Currently, there is no workaround for this issue. link:https://issues.redhat.com/browse/MTA-5398[(MTA-5398)]