...
Before this change, when you analyzed Java binary applications, {ProductShortName} incorrectly classified some open source dependencies as internal dependencies, it was then decompiled and analyzed, that caused the analysis to take longer to complete. With this update, the location of the Maven index file is passed to the Java provider to determine if an embedded dependency is open source or internal. This fix resolved the two following issues:
...
{ProductShortName} did not trigger rules to match import statements that contain a wildcard when analyzing a Java application. As a result, specific incidents were not detected in the report after the analysis. With this release, this issue is fixed. 
+
