...
.Analysis of binary applications classify open source dependencies

When you analyzed Java binary applications, {ProductShortName} incorrectly classified  some open-source dependencies as internal dependencies. With this version, this issue if fixed. This leads to {ProductShortName} classifying external open source dependencies correctly after a source+dependency binary analysis. link:https://issues.redhat.com/browse/MTA-6357[MTA-6357]

.{ProductShortName} {CLIName} parses the `VCP_SERVICES` environment variable correctly for Cloud Foundry applications with service binding

The discovery process of Cloud Foundry (CF) applications with service bindings by using live connection failed in {ProductShortName} {CLIName}. The {ProductShortName} {CLIName} did not parse the `VCAP_SERVICES` environment variables correctly to handle service binding information for an array of services. With this version, this issue is fixed. As a result, {ProductShortName} successfully completes the discovery of CF applications with service bindings. link:https://issues.redhat.com/browse/MTA-6399[MTA-6399]