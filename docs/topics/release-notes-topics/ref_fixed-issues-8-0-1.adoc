...
Analysis of binary applications classifies open source dependencies accurately::
...
{ProductShortName} did not trigger rules to match import statements that contain a wildcard when analyzing a Java application. Consequently, specific incidents were not detected in the report after the analysis. With this release, this issue is fixed. 
+
