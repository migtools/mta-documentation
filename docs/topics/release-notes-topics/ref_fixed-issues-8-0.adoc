...
Before this update, when you generated a deployment manifest by using the Helm template, {ProductShortName} ignored the default values in the `values.yaml` file packaged with the Helm chart. As a result, only the values from the `values.yaml` file created after running the `mta-cli discover` command were used. With this update, the default `values.yaml` file is merged with the discover `values.yaml` file.
+
