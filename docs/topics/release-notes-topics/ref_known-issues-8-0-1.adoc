...
.Specifying target organizations for a Cloud Foundry discovery is not supported in {ProductShortName} UI

When running an application discovery by using the {ProductFullName} user interface (UI), you cannot specify target organizations for a Cloud Foundry (CF) instance. You can only specify target CF spaces.
...
.`METHOD_CALL` and `CONSTRUCTOR_CALL` do not match expected locations

Searches by using the `METHOD_CALL` and `CONSTRUCTOR_CALL` patterns do not match the expected methods and constructors. For example, `java.referenced: location: CONSTRUCTOR_CALLpattern: javax.servlet.UnavailableException(javax.servlet.Servlet,java.lang.String)` does not match `String str = "something"; Servlet s = exception1.getServlet(); UnavailableException exception2 = new UnavailableException(s, str);`.

In addition, searches with Fully Qualified Names (FQN) specified do not match, for example, `java.referenced: location: CONSTRUCTOR_CALLpattern: UnavailableException(Servlet,String)`.
