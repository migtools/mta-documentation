...
+
The provider condition is used to define a search query for a specific language provider. The provider condition also specifies the capability of the provider used to complete the search query.
+
The condition's general format is `<provider_name>.<capability>`. The condition also has inner fields to specify details of the search. The way you create a `provider` condition and its inner fields depends on the provider you use and its capabilities.
+
...
|`hasTags`
3+^| This is an inline list of string tags. See *Tag Actions* in xref:yaml-tag-actions_rule-yaml-actions[Tag Actions] for details on tag format.
