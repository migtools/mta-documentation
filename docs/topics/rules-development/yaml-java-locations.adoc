...
----
|ENUM_CONSTANT|Matches against enum constants.|abc
|RETURN_TYPE|Matches against a type being returned by a method.|abc
|VARIABLE_DECLARATION|Matches against a type being declared as a variable|abc
...
xref:yaml-annotation-inspection_java-conditions-capabilities[Annotation inspection]|abc
...
java.referenced:
  pattern: javax.xml*
----
|CONSTRUCTOR_CALL|Matches against a constructor.|abc
...
java.referenced:
  ...
  location: IMPORT
----
|PACKAGE|Matches on any usage of a package, be it in an import or used as part of a fully qualified name in the code.|abc
