...
[NOTE]
====
When you use the `java` provider for an analysis, the analysis results have lower precision for projects that cannot be built. Consequently, when {ProductShortName} is unable to build a `Maven` project, it falls back to parsing the `pom` file to get the list of dependencies.
====