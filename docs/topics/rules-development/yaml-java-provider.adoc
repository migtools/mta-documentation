...
[NOTE]
====
When you use the `java` provider for an analysis, the analysis results have lower precision for projects that cannot be built. Consequently, the analysis result is affected in other ways. For example, dependencies cannot be found and the analyzer inspects the `pom` file.
====