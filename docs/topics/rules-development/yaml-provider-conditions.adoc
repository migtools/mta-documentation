...
.7+|xref:yaml-java-provider_rule-provider-conditions[java]
.3+|referenced
|pattern
|Yes
...
.9+|xref:yaml-builtin-provider_rule-provider-conditions[builtin]
.3+|xml
|xpath
|Yes
a|Xpath query
...
.5+|xref:yaml-go-provider_rule-provider-conditions[go]
|referenced
|pattern
|Yes
a|Regular expression pattern.
...
.3+|xref:yaml-dotnet-provider_rule-provider-conditions[dotnet]
.2+|referenced
|pattern
|Yes
