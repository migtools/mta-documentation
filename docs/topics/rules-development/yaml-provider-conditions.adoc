...
.7+|
xref:yaml-java-provider_provider-conditions[java]
.3+|referenced
|pattern
|Yes
...
.9+|
xref:yaml-builtin-provider_provider-conditions[builtin]
.3+|xml
|xpath
|Yes
a|Xpath query
...
.5+|
xref:yaml-go-provider_provider-conditions[go]
|referenced
|pattern
|Yes
a|Regular expression pattern.
...
.3+|
xref:yaml-dotnet-provider_provider-conditions[dotnet]
.2+|referenced
|pattern
|Yes
