...
[role="_abstract"]
{ProductShortName} settings apply to every analysis that you perform by using the Visual Studio Code extension. The extension settings have the following categories:
...
.. Open the settings page by clicking `Extensions` > `MTA`.
+
