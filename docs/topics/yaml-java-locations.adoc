...
link:#yaml-java-locations_rules-development-guide[Annotation inspection])
* *METHOD*: matches against a given method declaration. It can be coupled
with an annotation match (see link:#yaml-java-locations_rules-development-guide[Annotation
inspection]).
