...
A provider detects the programming languages, frameworks, and tools used to build an application and creates default rulesets for each supported provider using the Language Server Protocol (LSP) accordingly. 
...
[[tabl-Supported-providers-and-rule-conditions]]
